...
mycursor.execute("CREATE TABLE IF NOT EXISTS controlPendienteParticular (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, nombre VARCHAR(20), fechaHastaVencer DATE, kmHastaVencer DOUBLE(10,2), patenteVehiculo VARCHAR(7), idRevision INT UNSIGNED, estaLeida BOOLEAN, FOREIGN KEY(patenteVehiculo) REFERENCES vehiculosParticular(patente), FOREIGN KEY(idRevision) REFERENCES revisionesVehiculoParticular(id));")

mycursor.execute("CREATE TABLE IF NOT EXISTS controlPendienteOrganizacion (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, nombre VARCHAR(20), fechaHastaVencer DATE, kmHastaVencer DOUBLE(10,2), patenteVehiculo VARCHAR(7), idRevision INT UNSIGNED, estaLeida BOOLEAN, FOREIGN KEY(patenteVehiculo) REFERENCES vehiculosOrganizacion(patente), FOREIGN KEY(idRevision) REFERENCES revisionesVehiculoOrganizacion(id));")
