...
mycursor.execute("CREATE TABLE IF NOT EXISTS usuariosParticular (nombre VARCHAR(20), apellido VARCHAR(25), dni VARCHAR(10), email VARCHAR(60), contraseña VARCHAR(30), cuil VARCHAR(13) PRIMARY KEY);")
...
mycursor.execute("CREATE TABLE IF NOT EXISTS revisionesVehiculoParticular (nombre VARCHAR(20) PRIMARY KEY, fechaUltRevision DATE, periodicidadRevision DATE, estado VARCHAR(15), patenteVehiculo VARCHAR(7), FOREIGN KEY(patenteVehiculo) REFERENCES vehiculosParticular(patente));")
